...
function ProductImage({ product, allowPreview }) { const [isPreviewOpen, setPreviewOpen] = useState(false); if (!allowPreview) return <img src={product.image} alt={product.name}/>; return <><button className="image-preview-trigger" onClick={() => setPreviewOpen(true)} aria-label={`Ver foto ampliada de ${product.name}`}><img src={product.image} alt={product.name}/></button>{isPreviewOpen && <div className="image-lightbox" role="dialog" aria-modal="true" aria-label={`Foto ampliada de ${product.name}`}><button className="image-lightbox-backdrop" onClick={() => setPreviewOpen(false)} aria-label="Cerrar foto ampliada"/><div className="image-lightbox-content"><button className="image-lightbox-close" onClick={() => setPreviewOpen(false)} aria-label="Cerrar foto ampliada">×</button><img src={product.image} alt={product.name}/></div></div>}</> }
