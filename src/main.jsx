...
function ProductImage({ product }) { const [isPreviewOpen, setPreviewOpen] = useState(false); return <><button className="image-preview-trigger" onClick={() => setPreviewOpen(true)} aria-label={`Ver foto ampliada de ${product.name}`}><img src={product.image} alt={product.name}/></button>{isPreviewOpen && <div className="image-lightbox" role="dialog" aria-modal="true" aria-label={`Foto ampliada de ${product.name}`}><button className="image-lightbox-backdrop" onClick={() => setPreviewOpen(false)} aria-label="Cerrar foto ampliada"/><div className="image-lightbox-content"><button className="image-lightbox-close" onClick={() => setPreviewOpen(false)} aria-label="Cerrar foto ampliada">×</button><img src={product.image} alt={product.name}/></div></div>}</> }

function ProductCard({ product, onChoose, chooseLabel = 'Ver pieza' }) { const [isOpen, setOpen] = useState(false); return <article className="product"><div className={`product-image ${product.tone}`}><ProductImage product={product}/>{product.tag && <span>{product.tag}</span>}<button onClick={onChoose} aria-label={`${chooseLabel}: ${product.name}`}>+</button></div><div className="product-info"><div><h3>{product.name}</h3><p>{product.category}</p></div><strong>{money.format(product.price)}</strong></div><button className="details-button" onClick={() => setOpen(!isOpen)} aria-expanded={isOpen}>{isOpen ? 'Ocultar detalles' : 'Detalles'} <span>{isOpen ? '−' : '+'}</span></button>{isOpen && <div className="piece-details"><div><span>Medidas</span><p>{product.dimensions || 'A confirmar'}</p></div><div><span>Capacidad</span><p>{product.capacity || 'A confirmar'}</p></div></div>}</article> }
