...
function Header({ path, navigate, openCart, count }) {
  ...
  return <header className="header">
    ...
    <nav className={isMenuOpen ? 'open' : ''}>
      ...
      <a href="/#trabajos" onClick={() => setMenuOpen(false)}>Últimos trabajos</a>
...
function Home({ navigate, products, latestWorks }) {
  return <main id="inicio">
    ...
    <section className="shop shop-preview"><div className="section-heading"><div><p className="eyebrow">LA TIENDA</p><h2>Piezas para todos los días.</h2></div><button className="text-link" onClick={() => navigate('/tienda')}>Ir a la tienda <ArrowIcon/></button></div><div className="product-grid">{products.slice(0, 3).map((product) => <ProductCard product={product} key={product.id} onChoose={() => navigate('/tienda')}/>)}</div></section>
    <section id="trabajos" className="works"><div className="section-heading"><div><p className="eyebrow">HECHO A MEDIDA</p><h2>Últimos trabajos.</h2></div></div><div className="works-grid">{latestWorks.map((work) => <img key={work.id} src={work.image} alt="Trabajo de cerámica realizado por Locamanía"/>)}</div></section>
    <section id="historia" className="story"><div className="story-photo"><img src="https://images.unsplash.com/photo-1495555961986-6d4c1ecb7be3?auto=format&fit=crop&w=1000&q=85" alt="Proceso artesanal de cerámica"/></div><div className="story-copy"><p className="eyebrow">DEL BARRO A TU CASA</p><h2>Hechas para ser usadas, queridas y recordadas.</h2><p>Cada pieza empieza como un gesto entre las manos. Aceptamos las marcas del proceso y celebramos las pequeñas diferencias que hacen a cada objeto irrepetible.</p></div></section>
  </main>
}
